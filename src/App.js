...
function App() {
  ...
  useEffect(() => {
    const fetchCountries = async () => {
      fetch(
        "https://disease.sh/v3/covid-19/countries?yesterday=true&twoDaysAgo=false&allowNull=true"
      )
  ...
  const onCountryChange = async (e) => {
    ...
    const requestVaccineData = async () => {
      await fetch(
        `https://disease.sh/v3/covid-19/vaccine/coverage/countries/${selectedCountry}?lastdays=3`
      )
        .then((response) => response.json())
        .then((data) => {
